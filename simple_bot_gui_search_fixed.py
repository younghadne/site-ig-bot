...
class SimpleBotGUI:
    ...
    def _ensure_in_followers_tab(self):
        """Make sure we're in the Followers tab, not Following tab - IMPROVED"""
        try:
            self.log_message("🔄 Ensuring we're in Followers tab...")
            time.sleep(2)  # Wait for UI to load
            
            # First, check what tab we're currently on
            ...
            if current_tab == "followers":
                ...
                self.log_message("⚠️ Currently in Following tab - switching to Followers")
    ...
    def _search_user(self, username):
        ...
        try:
            ...
            # Click on user in search results - look for text patterns
            self.log_message(f"🔍 Looking for '{username}' or similar patterns in search results...")
            
            # Try multiple patterns and selectors
            search_patterns = [
            ...
            for pattern in search_patterns:
                # Try different selectors for account results
                user_selectors = [
                    f"//android.widget.TextView[contains(@text, '{pattern}') and @resource-id='com.instagram.android:id/row_search_user_username']",
                    f"//android.widget.TextView[contains(@text, '{pattern}') and contains(@resource-id, 'username')]",
                    f"//android.widget.TextView[contains(@text, '{pattern}')]",
                    f"//android.view.View[contains(@text, '{pattern}')]",
                    f"//android.widget.Button[contains(@text, '{pattern}')]"
                ]
                
                for selector in user_selectors:
                    try:
                        if selector.startswith("//"):
                            user_results = self.device.xpath(selector)
                        else:
                            user_results = self.device(resourceId=selector)
                        
                        if user_results.exists:
                            # Get the first matching result
                            if hasattr(user_results, '__iter__'):
                                user_result = user_results[0]
                            else:
                                user_result = user_results
                            
                            # Get the actual text to verify
                            actual_text = user_result.get_text()
                            self.log_message(f"✅ Found text: '{actual_text}' with pattern: '{pattern}'")
                            user_result.click()
                            time.sleep(3)
                            return True
                    except Exception as e:
                        self.log_message(f"⚠️ Selector failed: {selector} - {str(e)}")
                        continue
            
            # Try to find any text that contains the username
            self.log_message(f"🔄 Looking for any text containing '{username}'...")
            all_text_elements = self.device(className="android.widget.TextView")
            for element in all_text_elements:
                try:
                    text = element.get_text()
                    if text and username.lower() in text.lower():
                        self.log_message(f"✅ Found matching text: '{text}'")
                        element.click()
                        time.sleep(3)
                        return True
                except:
            ...
            # Try coordinate-based approach - click on first few search results
            self.log_message("🔄 Trying coordinate-based clicks on search results...")
            for y_pos in [200, 250, 300, 350]:  # Try different positions
                try:
                    self.device.click(540, y_pos)
                    time.sleep(2)
                    
                    # Verify we're on the right profile by checking username
                    current_username = self._get_current_profile_username()
                    if current_username and username.lower() in current_username.lower():
                        self.log_message(f"✅ Successfully navigated to profile containing '{username}': @{current_username}")
                        return True
                    else:
            ...
            self.log_message(f"❌ Could not find any account matching '{username}'")
            return False
    ...
    def _process_followers(self, account, max_followers, follow_limit, like_limit):
        """Process followers from followers list - keep following until daily limit reached"""
        ...
        try:
            ...
            self.log_message(f"🎯 Will continue until daily follow limit: {follow_limit}")
            
            # Keep following until daily limit is reached or max_followers reached
            i = 0
            while self.bot_running and self.stats['followers_gained'] < follow_limit and i < max_followers * 10:  # Allow more iterations
                ...
                try:
                    if stay_in_followers:
                        ...
                        if self._follow_follower_from_list():
                            ...
                            self.log_message(f"✅ Followed follower #{i+1} - Total: {self.stats['followers_gained']}/{follow_limit}")
                            self.update_stats_display()
                        else:
                    ...
                    self.log_message(f"⬇️ Scrolling down to see more followers... Progress: {self.stats['followers_gained']}/{follow_limit}")
                    ...
                    # Safety check - if we've processed too many without follows, break
                    if i > max_followers * 5 and followers_processed < 5:
                        self.log_message("⚠️ Too many attempts with few follows - may be at end of list")
                        break
            ...
            if self.stats['followers_gained'] >= follow_limit:
                ...
                self.log_message(f"🛑 Stopped. Total followed: {followers_processed}")
